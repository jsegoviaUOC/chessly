    ...
    public void Create(int xAxis, int yAxis)
    {
        ...
        for (int y = 0; y < yLimit; y++)
        {
            for (int x = 0; x < xLimit; x++)
            {
                ...
                if ((x + y) % 2 == 0)
                {
                    mAllCells[x, y].GetComponent<Image>().color = EditorManager.getColor(EditorManager.optionsData.colors.boardColor + "Dark");
                }
                else
                {
                    mAllCells[x, y].GetComponent<Image>().color = EditorManager.getColor(EditorManager.optionsData.colors.boardColor + "Light");
                }
            }
        }
    }
